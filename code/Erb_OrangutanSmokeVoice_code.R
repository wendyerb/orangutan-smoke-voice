...
setwd('/Users/Me/github/orangutan-smoke-voice/data')

## Read files
Ncall_PreSmo <- read.csv('N.LC_PreSmoke.csv')
Ncall_PrePos <- read.csv('N.LC_PrePost.csv')

duration_PreSmo <- read.csv('duration_PreSmoke.csv')
duration_PrePos <- read.csv('duration_PrePost.csv')
...
## Code for general linear mixed models & model residuals (use this for all variables except those listed below)
...
## Code for generalized linear mixed models (Poisson, log) & model residuals (use this for # long calls)
...
## Code for generalized linear mixed models (binomial, logit) & model residuals (use this for biphonation, and deterministic chaos)
